    ...
    return (
        <div className={`md:pt-20 ${data.bgColor} py-24 rounded-tl-[6.5rem] rounded-tr-[6.5rem]`}>
            <div className="w-[85%] m-auto flex flex-col gap-2 text-center">
                ...
                <div className={`w-[100%] mt-20 border-2 rounded-[3rem] ${data.borderColor} ${data.hoverColor} ${data.subTextHoverColor} ${data.subTextColor}`}><h3 className=' md:w-[60%] m-auto py-6 text-[3rem] md:text-[4rem] font-[700] leading-tight'>{data.subtext}<span className={`uppercase ${data.tagColor}`}><span className="text-white">{data.tag2}</span> {data.tag}</span></h3></div>
            </div>
        </div>
    )
}
